import { statusIconHtml } from './icons.js'
...
export function renderMerakMarkdown(markdown) {
  ...
  renderer.blockquote = function (token) { return `<aside class="mp-alert mp-alert--info" role="note"><span class="mp-alert__icon" aria-hidden="true">${statusIconHtml('info')}</span><div class="mp-alert__content">${this.parser.parse(token.tokens)}</div></aside>\n` }
